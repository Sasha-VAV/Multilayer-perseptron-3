...
class Network:
    ...
    def back_propagation(self, true_data):
        ...
        for i in range(len(self.size)-2,-1,-1):
            for k in range(size[i]):
                for j in range(size[i+1]):
                    value = alpha * self.network[i][j]*(self.network[i+1][j]-true_data[j])
                    self.weights[i][j][k]-= value * self.func.df(func,self.backup_network[i+1][j])
                    self.bios[i][j] -= alpha *(self.network[i+1][j]-true_data[j])* self.func.df(func,self.backup_network[i+1][j])
...
for j in range(num_of_laps):
    ...
    try:
        if j%(num_of_laps//20)==0:
            print(f'Epoch {j}: avg err: {average_error/num_of_learning_examples}; avg real success: {num_of_real_success/num_of_learning_examples}; avg success: {num_of_success/num_of_learning_examples}')
    except:
        print(f'Epoch {j}: avg err: {average_error / num_of_learning_examples}; avg real success: {num_of_real_success / num_of_learning_examples}; avg success: {num_of_success / num_of_learning_examples}')
    learn_file.close()